...
class ChannelMember(Document):
    ...
    def before_insert(self):
        ...
        if frappe.db.exists("Channel Member", {"channel_id": self.channel_id, "user_id": self.user_id}):
        ...
        if frappe.db.count("Channel Member", {"channel_id": self.channel_id}) == 0:
            self.is_admin = 1

    def after_delete(self):
        if frappe.db.count("Channel Member", {"channel_id": self.channel_id}) == 0 and frappe.db.get_value("Channel", self.channel_id, "type") == "Private":
            frappe.db.set_value("Channel", self.channel_id,
                                "is_archived", 1)
        if self.get_admin_count() == 0 and frappe.db.count("Channel Member", {"channel_id": self.channel_id}) > 0:
            first_member = frappe.db.get_value("Channel Member", {
                                               "channel_id": self.channel_id}, ["name"], as_dict=1, order_by="creation")
            frappe.db.set_value("Channel Member",
                                first_member.name, "is_admin", 1)

    def on_trash(self):
        # if the leaving member is admin, then the first member becomes new admin
        if self.is_admin == 1 and frappe.db.count("Channel Member", {"channel_id": self.channel_id}) > 0:
            first_member = frappe.db.get_value("Channel Member", {
                                               "channel_id": self.channel_id}, ["name"], as_dict=1, order_by="creation")
            frappe.db.set_value("hannel Member",
                                first_member.name, "is_admin", 1)
        if not self.check_if_user_is_member():
            frappe.throw(
    ...
    def check_if_user_is_member(self):
        is_member = True
        channel = frappe.db.get_value("Channel", self.channel_id, [
        ...
        if channel.type == "Private":
            ...
            if channel.owner == frappe.session.user and frappe.db.count("Channel Member", {"channel_id": self.channel_id}) == 0:
                # User is the owner of a channel and there are no members in the channel
                pass
            elif frappe.db.exists("Channel Member", {"channel_id": self.channel_id, "user_id": frappe.session.user}):
    ...
    def get_admin_count(self):
        return frappe.db.count("Channel Member", {"channel_id": self.channel_id, "is_admin": 1})
